...
# Repeat FCM while master has not accepted (Celery).
# Non-SOS (standard / towing / custom): every N seconds (default 60 = 1 minute).
# SOS: every N seconds (default 5).
MASTER_NEW_ORDER_REMINDER_ENABLED = os.environ.get(
    'MASTER_NEW_ORDER_REMINDER_ENABLED', 'true'
).lower() in ('1', 'true', 'yes')
MASTER_NEW_ORDER_REMINDER_SECONDS = int(os.environ.get('MASTER_NEW_ORDER_REMINDER_SECONDS', '60'))
MASTER_NEW_ORDER_REMINDER_SOS_SECONDS = int(os.environ.get('MASTER_NEW_ORDER_REMINDER_SOS_SECONDS', '5'))
# Safety cap so a stuck pending offer cannot spam forever.
...
SPECTACULAR_SETTINGS = {
    ...
    'SERVERS': [
        {'url': 'https://api.autohandy.app/', 'description': 'Production server'},
