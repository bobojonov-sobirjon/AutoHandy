...
class MasterProfileView(APIView):
    ...
    @extend_schema(
        ...
        **Примечания:**
        ...
        - **Один пользователь — одна мастерская:** если у аккаунта уже есть профиль `Master`, повторное создание вернёт **400**.
        - Можно создать мастерскую вообще без данных и заполнить потом через PUT/PATCH (только если профиля ещё нет)
        """,
