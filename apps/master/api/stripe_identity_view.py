...
class MasterIdentityStartDataSerializer(serializers.Serializer):
    ...
    verification_session_id = serializers.CharField(required=False, help_text='Alias of session id (Android SDK).')
    client_secret = serializers.CharField(allow_null=True, help_text='iOS / web-friendly secret.')
    ephemeral_key_secret = serializers.CharField(
        allow_null=True,
        required=False,
        help_text='Android IdentityVerificationSheet.present(ephemeralKeySecret=...).',
    )
    url = serializers.CharField(allow_null=True, required=False, help_text='Optional hosted VerificationSession URL.')
